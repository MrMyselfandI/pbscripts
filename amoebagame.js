...
module.exports = function() {
    ...
    var init = function() {
        ...
        SESSION.channels(id).master = "Mr. Perry";
    };
    ...
    var beforeChatMessage = function(src, message, chan) {
        ...
        if (role != species.length - 1) {
            ...
            if (battles[role] && battles[role] != src && sys.isInChannel(battles[role], id)) {
                ...
                sendChanAll("+EvolutionBot: Dois " + species[role] + "s, "
                    + sys.name(battles[role]) + " and " + sys.name(src) + " engaged in a fierce battle! "
                    + sys.name(winner) + " venceu e agora é um " + species[SESSION.users(winner).amoeba.role] + ". "
                    + sys.name(loser) + " perdeu e agora é um " + species[SESSION.users(loser).amoeba.role] + ".",
                id);
                delete battles[role];
            } else {
                battles[role] = src;
