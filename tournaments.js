...
function Tournament(channel)
{
    ...
    function parseTierAndCount(source, data) {
        ...
        if (isNaN(count) || count <= 2 || count > 256){
            sendPM(source, "Você precisa especificar a quantidade de vagas do torneio (maior que 3 e menor que 256).");
            return;
        }

        var tier = findTier(commandpart[0]);
        if (tier === null) {
            sendPM(source, "O server não reconhece a tier " + commandpart[0] + " .");
            return;
        }
        ...
        if (commandpart[2] === undefined) {
            ...
            if ((modeOfTier(tier) == "Doubles" || modeOfTier(tier) == "Triples" || singlesonlytiers.indexOf(tier) != -1) && !cmp(commandpart[2], modeOfTier(tier))) {
                sendPM(source, "A "+tier+" pode ser jogada apenas no modo " + modeOfTier(tier) + " !");
                return;
            }
    ...
    function start(source, data) {
        if (self.running) {
            sendPM(source, "Um torneio já está em andamento!");
            return;
        }
    ...
    function initTournament(starter, tier, count, mode) {
        ...
        wall(border);
        wall("*** Um torneio foi iniciado por " + self.starter + "! ***");
        wall("VAGAS: " + self.count);
        wall("TIER: " + self.tier);
        wall("MODO: " + self.mode);
        broadcast("CLÁUSULAS: " + tierClauses(self.tier).join(", "));
        wall("");
        advertise("*** Vá para o channel #" + sys.channel(self.channel) + " e digite /join ou !join para entrar neste torneio! ***");
            ...
            for (var x = 0; x < playerson.length; ++x) {
                ...
                if (sys.loggedIn(id) && poUser && poUser.tiers && poUser.tiers.indexOf(self.tier) != -1) {
                    if(sys.isInChannel(id, self.channel)) {
                        sys.sendHtmlMessage(playerson[x], "Um torneio com a tier "+ self.tier+" está começando, " + sys.name(playerson[x]) +"<ping/>!", self.channel);
                        continue;
                    } else if(sys.isInChannel(id, 0)) {
                        sys.sendHtmlMessage(playerson[x], "Um torneio com a tier "+ self.tier+" está começando, " + sys.name(playerson[x]) +"<ping/>!",0);
                        continue;
                    }
                    sys.sendHtmlMessage(id, "Um torneio com a tier "+ self.tier+" está começando, " + sys.name(playerson[x]) +"<ping/>!");
                }
            }
        }
        self.running = true;
    ...
    function viewQueue(source) {
        if (self.queue.length > 0) {
            sendPM(source, "Os seguintes torneios estão na lista de espera: " +
                self.queue.map(function(e) {
                    return e.starter + " adicionou um torneio com a tier '" + e.tier + "' "+(e.mode != modeOfTier(e.tier) ? "("+e.mode+")" : "")+"com tamanho inicial de " + e.count;
                }).join(", "));
        } else {
            sendPM(source, "A lista de espera de torneios está vazia.");
        }
    }
    ...
    function queue(source, data) {
        ...
        var QUEUE_MAX_SIZE = 4;
        if (self.queue.length > QUEUE_MAX_SIZE) {
            sendPM(source, "A lista de espera já se encontra com " + QUEUE_MAX_SIZE + " torneios.");
            return;
        }
        for (var i = 0, l = self.queue.length; i < l; ++i) {
            if (self.queue[i].tier == res.tier) {
                sendPM(source, "Esta tier já está na lista de espera.");
                return;
            }
        }
        ...
        broadcast(res.starter + " adicionou uma nova tier a lista de espera!");
    ...
    function rmQueue(source, data) {
        ...
        if (tier === null) {
            sendPM(source, "" + data + " não é uma tier.");
            return;
        }
        ...
        for (var i = 0, l = self.queue.length; i < l; ++i) {
            if (self.queue[i].tier == tier) {
                ...
                sendPM(source, "Uma tier foi removida da lista de espera.");
                return;
            }
        }
        sendPM(source, "Esta tier não está na lista de espera.");
    }
    ...
    function join(source) {
        ...
        if (isInTour(name)) {
            sendPM(source, "Você já se inscreveu no torneio!");
            return;
        }
        ...
        if (!sys.hasTier(source, self.tier)){
            sendPM(source, "Neste momento, você não se encontra na tier " + self.tier + " . Troque sua tier para " + self.tier + " e assim estará apto a se inscrever.");
            return;
        }

        self.ips.push(sys.ip(source));
        addEntrant(name);
        broadcast("●PBSTourBot: " + name + " se inscreveu no torneio! " + remainingEntrants()  + " vagas restantes!");
        ...
        if (remainingEntrants() > 0 && remainingEntrants() <= self.count/8) {
            ...
            if ((Date.now() - self.startTime)/1000 < 40 + self.count*5) {
                ...
                broadcast("●PBSTourBot: Este torneio está aberto para " + self.count + " jogadores!");
            } else {
                while (remainingEntrants() > 0) {
                    name = freeSub();
                    addEntrant(name);
                }
                broadcast("●PBSTourBot: Substitutos foram adicionados e assim o torneio foi iniciado!");
            }
        }
    ...
    function subme(source) {
        if (!playingPhase()) {
            sendPM(source, "Espere um torneio ter inicio!");
            return;
        }
        ...
        if (self.round != 1) {
            sendPM(source, "Vagas para substitutos são válidos apenas na 1ª rodada!");
            return;
        }
        ...
        if (typeof SESSION.users(source).battles === "object" && Object.keys(SESSION.users(source).battles).length > 0) {
            sendPM(source, "Você não pode se inscrever enquanto batalha!");
            return;
        }
        ...
        if (placeholder) {
            self.ips.push(sys.ip(source));
            broadcast("●PBSTourBot: " + players[0] + " se inscreveu no torneio! (Adeus " + players[1] + "!)");
            switchPlayers(players);
        } else {
            sendPM(source, "Não mais substitutos!");
        }
    }
    ...
    function unjoin(source) {
        if (!self.running) {
            sendPM(source, "Espere até o torneio ter início.");
            return;
        }
    ...
    function viewround(source) {
        if (!playingPhase()) {
            sendPM(source, "Você não pode usar este comando agora pois não há um chaveamento de torneio ativo.");
            return;
        }
        ...
        sendPM(source, "", false);
        sendPM(source, "*** RODADA " + self.round + " DO TORNEIO ***", false);

        var i;
        if (self.battlesLost.length > 0) {
            sendPM(source, "", false);
            sendPM(source, "*** BATALHAS ENCERRADAS ***", false);
            sendPM(source, "", false);
            for (i = 0; i < self.battlesLost.length; i+= 2) {
                sendPM(source, self.battlesLost[i] + " venceu " + self.battlesLost[i+1], false);
            }
            sendPM(source, "", false);
        }
        ...
        if (self.battlers.length > 0) {
            if (self.battlesStarted.indexOf(true) != -1) {
                sendPM(source, "", false);
                sendPM(source, "*** OBATALHAS EM ANDAMENTO ***", false);
                sendPM(source, "", false);
            ...
            if (self.battlesStarted.indexOf(false) != -1) {
                sendPM(source, "", false);
                sendPM(source, "*** BATALHAS FALTANDO INICIAR ***", false);
                sendPM(source, "", false);
                for (i = 0; i < self.battlers.length; i+=2) {
                    if (!self.battlesStarted[i/2]) {
                        sendPM(source, padd(self.entrants[self.battlers[i]]) + " VS " + self.entrants[self.battlers[i+1]], false);
        ...
        if (self.members.length > 0) {
            sendPM(source, "", false);
            sendPM(source, "*** JOGADORES NA PRÓXIMA RODADA ***", false);
            sendPM(source, "", false);
            var s = [];
    ...
    function dq(source, name) {
        if (!self.running) {
            sendPM(source, "Espere até que o torneio se inicie.");
        }
        ...
        if (isInTour(name)) {
            if (self.phase == "entry" || self.members.indexOf(name.toLowerCase()) >= 0) {
                removeEntrant(name);
                broadcast("●PBSTourBot: " + name + " foi removido do torneio por " + authority + "!");
            ...
            sendPM(source, name + " não está no torneio.");
        }
    }
    ...
    function push(source, name) {
        ...
        if (self.phase == "playing") {
            broadcast(name + " foi adicionado ao torneio por " + sys.name(source) + ".");
        } else if (self.phase == "entry") {
            broadcast(name + " foi adicionado ao torneio por " + sys.name(source) + ". " + remainingEntrants() + " vagas restantes!");
    ...
    function cancelBattle(source, name) {
        ...
        if (isBattling(name)) {
            sendPM(source, name + " pode fechar sua batalha atual e começar outra.");
            sendPM(sys.id(name), "Você pode fechar sua batalha agora e começar outra.");
            sendPM(sys.id(tourOpponent(name)), "Você pode fechar sua batalha agora e começar outra.");
    ...
    function sub(source, data) {
        ...
        if (!isInTour(players[0]) && !isInTour(players[1])) {
            sendPM(source, "Nenhum destes está no torneio.");
            return;
        }

        broadcast(players[0] + " e " + players[1] + " tiveram seus lugares trocados no torneio por "  + sys.name(source));
        switchPlayers(players);
    }
    ...
    function changeCount(source, data) {
        if (self.phase != "entry") {
            sendPM(source, "Este comando só pode ser usado durante a fase de inscrições.");
            return;
        }
        ...
        if (isNaN(count) || count <= 2 || count > 256){
            sendPM(source, "Especifique um tamanho para o torneio (maior que 3 e menor que 256 jogadores).");
            return;
        }

        if (count < memberCount()) {
            sendPM(source, "Há mais pessoas inscritas no torneio que este número!");
            return;
        }
        ...
        broadcast("");
        broadcast(border);
        broadcast("●PBSTourBot: " + sys.name(source) + " trocou o número de vagas do torneio para " + count + "!");
        broadcast(border);
        broadcast("");
    ...
    function endTour(source, data) {
        if (self.running) {
            ...
            broadcast("");
            broadcast(border);
            broadcast("●PBSTourBot: O torneio foi cancelado por " + sys.name(source) + "!");
            broadcast(border);
            broadcast("");
            
            scheduleTournamentFromQueue();
        } else {
            sendPM(source, "Você não pode encerrar um torneio se não existe um em andamento.");
        }
    }
    ...
    function pingAuth() {
        ...
        for (var i = 0; i < p.length; ++i) {
            ...
            if (self.main && sys.auth(id) > 0 || SESSION.channels(self.channel).isChannelOperator(id)) {
                sys.sendHtmlMessage(id, "●PBSTourBot: Há batalhas que não foram iniciadas!<ping />" ,self.channel);
    ...
    function roundPairing() {
        ...
        if (memberCount() == 1) {
            ...
            broadcast(border);
            broadcast("");
            broadcast("O vencedor do torneio com a tier " + self.tier.toUpperCase() + " foi : " + casedName(firstPlayer()));
            broadcast("");
            broadcast("*** Parabéns, " + casedName(firstPlayer()) + ", pelo seu sucesso! ***");
            broadcast("");
            broadcast(border);
            broadcast("");
        ...
        if (!finals) {
            broadcast("");
            broadcast(border);
            broadcast("*** Rodada " + self.round + " do " + self.tier + " torneio ***");
            broadcast("");
        }
        else {
            wall("");
            wall(border);
            wall("*** FINAL DO TORNEIO DA TIER " + self.tier.toUpperCase() + " ***");
            advertise("*** Vá para o channel #" + sys.channel(self.channel) + " acompanha-lá! ***");
            wall("");
        }
        ...
        if (self.members.length > 0) {
            broadcast("");
            broadcast("*** " + casedName(self.members[0]) + " foi selecionado aleatoriamente para avançar para o próximo round! ***");
        }
        ...
        sys.delayedCall(function RemoveInactiveAndSubs() {
            if (self.running && self.round == current_round) {
                ...
                while (null !== (placeholder = findPlaceholder())) {
                    ...
                    broadcast("●PBSTourBot: " + placeholder + " foi removido do torneio!");
                }
                ...
                for (var i = 0; i < unstarted; i+=2) {
                    ...
                    if (online[0] && online[1]) {
                        broadcast("●PBSTourBot: Existe um problema com a partida entre " + unstarted[i] + " e " + unstarted[i+1] +".");
                        pingAuth();
                    } else if (!online[0] && !online[1]) {
                        broadcast("●PBSTourBot: " + unstarted[i] + " e " + unstarted[i+1] +" estão offline.");
                        pingAuth();
                    } else {
                        ...
                        broadcast("●PBSTourBot: " + loser + " foi removido do torneio por estar offline!");
                    }
                }
            }
        }, 240);
    ...
    function endBattle(winner, loser) {
        ...
        if (self.battlers.length !== 0 || self.members.length > 1) {
            broadcast("");
            broadcast(border);
            broadcast("●PBSTourBot: " + winner + " avança a próxima rodada.");
            broadcast("●PBSTourBot: " + loser + " está fora do torneio.");
        }
        if (self.battlers.length > 0) {
            broadcast("*** " + self.battlers.length/2 + " partida(s) faltando.");
            broadcast(border);
            broadcast("");
            return;
        }
    ...
    function beforeChallenge(source, dest, clauses, rated, mode, team, destTier) {
        ...
        if (isInTour(name1)) {
            if (!areOpponents(name1, name2)) {
                sendPM(source, "Este usuário não é o seu adversário no torneio.");
                return true;
            }
            var srcTier = sys.tier(source, team);
            if (srcTier != destTier || !cmp(srcTier, self.tier)) {
                sendPM(source, "Você necessita estar na tier " + self.tier + " para batalhar neste torneio.");
                return true;
            }
            ...
            if (clauses != tierClauses) {
                ...
                if (extra.length > 0)
                    sendPM(source, "Você tem que remover estas cláusulas do seu desafio: " + extra.join(", "));
                if (missing.length > 0)
                    sendPM(source, "Você tem que adicionar estas cláusulas ao seu desafio: " + missing.join(", "));
                if (extra.length > 0 || missing.length > 0)
                    return true;
            }
            if ((self.mode == "Singles" && mode !== 0) || (self.mode == "Doubles" && mode != 1) || (self.mode == "Triples" && mode != 2)) {
                sendPM(source, "Sua batalha tem que ser jogada no formato "+self.mode+" . Troque isso na janela de desafio.");
                return true;
            }
            if (self.phase == "finals" && hasClause(clauses, DISALLOW_SPECS)) {
                sendPM(source, "Não se usa \"disallow specs\" em finais.");
                return true;
            }
    ...
    function afterChannelJoin(source, channel) {
        if (self.phase == "entry") {
            sys.sendMessage(source,"*** Um torneio com a tier " + self.tier + " está em fase de inscrições, " + remainingEntrants() + " vagas faltando!",channel);
    ...
    this.announceInit = function announceInit() {
        broadcast("Torneios estão agora ativos em #" + sys.channel(self.channel) + "!");
    };
        ...
        if (module.tournaments[channel] !== undefined) {
            if (command in module.tournaments[channel].commands) {
                ...
                if (sys.auth(source) === 0 && !SESSION.users(source).megauser && !isChanOp) {
                    sys.sendMessage(source, "Você não pode usar este comando.");
                    return true;
                }
            ...
            if (command == "disabletours" && (sys.auth(source) >= 2 || SESSION.channels(channel).isChannelAdmin(source))) {
                ...
                tourneybot.sendAll("Torneios foram desativados.", channel);
